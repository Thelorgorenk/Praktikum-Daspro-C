...
typedef struct Card
{
    char nama[40], kelas[12], rarity[7], gender[15], align[50], nprank[3];
    int level, nplvl, atk, hp, blvl, rarin, clasin, nprin, index;
} card;
card kartu[10000], kartu2[10000];
...
int main()
{
    int n, i, uji, num = 0, clasnum = 0, gennum = 0, rarnum = 0, atknum = 0, rarii[10]={0},npnum=0,lvnum=0,hpnum=0,blnum=0,rankii=0,rarnumz=0, atk1[5]={0}, atk2[5] = {0}, hp1[5]={0}, hp2[5] = {0}, npl1[5] = {0}, npl2[5] = {0}, bl1[5] = {0}, bl2[5] = {0}, lv1[5] = {0}, lv2[5] ={0};
bool bgen=0, ranki = 0, nleve = 0, rar = 0, kles = 0;
char gen[20], klez[13], cmd[7], cmd1[20], fnrank[5];
    scanf("%d", &n);
    getchar();
    for(i=0; i<n; i++)
    {
        ...
        if(cmp(cmd, "SortBy"))
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if ((kartu[i].level >= lv1[0] && kartu[i].level <= lv2[0]) && (kartu[i].atk >= atk1[0] && kartu[i].atk <= atk2[0]) && (kartu[i].nplvl >= npl1[0] && kartu[i].nplvl <= npl2[0]) && (kartu[i].hp >= hp1[0] && kartu[i].hp <= hp2[0]) && (kartu[i].blvl >= bl1[0] && kartu[i].blvl <= bl2[0]))
                {
                    ...
                    {
                    kartu2[num].index = kartu[i].index;kartu2[num].level = kartu[i].level;strcpy(kartu2[num].nama, kartu[i].nama);strcpy(kartu2[num].rarity, kartu[i].rarity);strcpy(kartu2[num].kelas, kartu[i].kelas);
                    num++;
                    }
                    // class, nprank, rarity
            ...
            cout << "Case #" << sorting << "\n";
            for (int i = 0; i < num; i++) cout << kartu2[i].index << ": " << kartu2[i].nama << ", " << kartu2[i].rarity << " " << kartu2[i].kelas << ", Level " << kartu2[i].level << "\n";
            num = 0; bgen=0; clasnum = 0; gennum = 0;ranki=0;nleve=0;rar=0;rarnum=0;kles=0;atknum=0;lvnum=0,npnum=0;hpnum=0;blnum=0;rankii=0;memset(&kartu2, 0, sizeof(kartu2));memset(rarii, 0, sizeof(rarii));memset(atk1, 0, sizeof(atk1));memset(atk2, 0, sizeof(atk2));memset(hp1, 0, sizeof(hp1));memset(hp2, 0, sizeof(hp2));memset(lv1, 0, sizeof(lv1));memset(lv2, 0, sizeof(lv2));memset(npl1, 0, sizeof(npl1));memset(npl2, 0, sizeof(npl2));memset(bl1, 0, sizeof(bl1));memset(bl2, 0, sizeof(bl2));
        }
        if(cmp(cmd, "Gender"))
        {
            bgen = 1;
